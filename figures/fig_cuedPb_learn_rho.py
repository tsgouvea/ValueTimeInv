...
while (iTrial+1) < nTrials:
    ...
    if (iTrial + 1) % int(nTrials/100) == 0:
        print("iTrial:{:5.0f}, rho {:1.2f}, delta_rho {:3.2f}".format(iTrial + 1, rhoHat, delta_rho))
    ...
    # rhoHat = max(1e-6, rhoHat + learnRateRho * delta_rho)
...
hf_summ.savefig('fig_cuedPb_learn_rho.png')
