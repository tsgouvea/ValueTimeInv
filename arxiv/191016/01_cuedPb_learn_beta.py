...
ITI = .1
rewMag = 1
stim_set = np.linspace(50,100,11)/100#
stim_noise = .2
...
mySession.loc[:, 'P(B)'] = np.random.choice(stim_set, nTrials) # here to be interpreted as p(Reward)
...
#% Initialization
mySession.loc[0, 'rho'] = .45
mySession.loc[0, 'beta'] = .01
...
while (iTrial+1) < nTrials:
    ...
    if (iTrial + 1) % int(nTrials/100) == 0:
        print("iTrial:{:5.0f}, beta {:1.2f}, delta_beta {:3.2f}, rho {:1.2f}".format(iTrial + 1, betaHat, delta_beta, rhoHat))
    ...
    k = max(1e-6, betaHat * np.log(x / (betaHat * rhoHat)))
    ...
    # A'
    delta_rho = (r * rewMag) / tau - rhoHat
    # num = (r*(f-betaHat) + (1-r) * (x * k / ((x + (1 - x) * np.exp(k / betaHat)))))
    # den = (betaHat ** 2)
    # print('delta_beta: num {:3.2e} / den {:3.2e})'.format(num,den))
    # # delta_beta =  num / den
    # delta_beta = r * (f-betaHat) / betaHat**2# if r else 0# k*x/(x + (1-x)*np.exp(k/betaHat))
    # # delta_beta = delta_beta
    #
    # # delta_beta = x*k*np.exp(-k/betaHat)/betaHat**2*(1-2*r)

    # # the one
    delta_beta = k / betaHat * np.exp(-k / betaHat) / betaHat * (
            (1 - r) * x / (1 - x + x * np.exp(-k / betaHat)) - r / (1 - np.exp(-k / betaHat)))

    if False:
        if True: #const15
            rhoHat = rhoHat + (1 - (1 - learnRateRho) ** tau) * delta_rho
        elif False:
            rhoHat = (1 - (1 - learnRateRho) ** tau) * (r * rewMag) / tau + (1 - learnRateRho) ** tau * rhoHat
        else:
            G = (r * rewMag) / tau
            rhoHat = G + (1 - learnRateRho) ** tau * (rhoHat - G)  #me correcting Const15
    else:
        rhoHat = rhoHat + learnRateRho * delta_rho
    # rhoHat = rhoHat + (1 - learnRateRho) ** tau * (G - rhoHat)  #Const15
    # rhoHat = rhoHat + (1 - learnRateRho) ** tau * (G - rhoHat)  # Const15
    betaHat = betaHat + learnRateBeta * delta_beta

    rhoHat = max(1e-6,rhoHat)
    betaHat = max(1e-6, betaHat)
...
#%
x_bins = stim_set#np.unique(abs((stim_set-stim_set.mean())))
...
sns.regplot(x='P(B)', y='waitingTime', data=mySessionLate, logistic=False, ci=None,
            ax=ha_waitingTime[0,0],x_bins=stim_set, truncate=True)
...
sns.regplot(x='P(B)', y='waitingTime', ax=ha_waitingTime[0,1],truncate=True,
            ...
            color='xkcd:leaf green', ci=None, x_bins=stim_set, line_kws={'label':'correct'})
sns.regplot(x='P(B)', y='waitingTime', ax=ha_waitingTime[0,1],truncate=True,
            data=mySessionLate.loc[np.logical_not(mySessionLate.isCorrect), :],
            color='xkcd:brick', ci=None, x_bins=stim_set, line_kws={'label':'error'})
x = mySession.loc[iTrial, 'P(B)']
betaHat = mySession.loc[iTrial, 'beta']
rhoHat = mySession.loc[iTrial, 'rho']
f = mySession.loc[iTrial, 'feedbackTime']
...
for i, stim in enumerate(stim_set):
    ha_waitingTime[1,0].plot(mySession.waitingTime[mySession.loc[:,'P(B)'] == stim])
    ha_waitingTime[1,0].set_xlabel('trial #')
    ha_waitingTime[1,0].set_ylabel('k \n (waitingTime)')
    ha_waitingTime[1,1].hist(mySession.waitingTime[mySession.loc[:,'P(B)'] == stim],bins=bins,histtype='step')
...
sns.regplot(x='P(B)', y='isCorrect', ax=ha_signatures[2], truncate=True,
            ...
            color='xkcd:dark blue', ci=None, x_bins=stim_set, line_kws={'label': 'long WT'})
sns.regplot(x='P(B)', y='isCorrect', ax=ha_signatures[2], truncate=True,
            data=mySessionLate.loc[np.logical_not(mySessionLate.long_wt), :], logistic=False,
            color='xkcd:light blue', ci=None, x_bins=stim_set, line_kws={'label': 'short WT'})
