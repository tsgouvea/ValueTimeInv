...
while (iTrial+1) < nTrials:
    ...
    # # delta_mHat = expit(-mHat * abs(xHat - bHat)) * abs(xHat - bHat) * (r + (1-r)*(expit(mHat * abs(xHat - bHat)) * (1 - np.exp(-k/betaHat)))/(expit(mHat * abs(xHat - bHat)) * (1 - np.exp(-k/betaHat)) - 1))
    ...
    # delta_beta = k * np.exp(-k / betaHat) / betaHat**2 * (
    #         (1 - r) * pRew / (1 - pRew + pRew * np.exp(-k / betaHat)) - r / (1 - np.exp(-k / betaHat)))
