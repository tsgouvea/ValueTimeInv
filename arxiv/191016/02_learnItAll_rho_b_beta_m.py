...
from locallib import makephi, pnorm, truncExp, grad_beta, grad_psyc
...
while (iTrial+1) < nTrials:
    if (iTrial + 1) % int(nTrials/100) == 0:
        print("iTrial:{:5.0f}, b {:1.2f}, m {:1.2f}, rho {:1.2f}, beta {:1.2f}".format(iTrial + 1, bHat, mHat, rhoHat, betaHat))
    ...
    delta_bHat_and_mHat = expit(-mHat * abs(xHat - bHat)) * (
            r + (1 - r) * (expit(mHat * abs(xHat - bHat)) * (1 - np.exp(-k / betaHat))) / (
            expit(mHat * abs(xHat - bHat)) * (1 - np.exp(-k / betaHat)) - 1))

    # delta_bHat = delta_bHat_and_mHat * mHat * np.sign(bHat - xHat)
    # delta_mHat = delta_bHat_and_mHat * abs(xHat - bHat)
    delta_bHat, delta_mHat = grad_psyc(bHat, mHat, x, r, k, betaHat)
        # abs(xHat - bHat) * (r - expit(mHat * abs(xHat - bHat)))
    ...
    delta_beta = grad_beta(betaHat, pRew, k, r)
...
mySessionLate.loc[:, 'prev_long_wt'] = np.hstack((False, mySessionLate.long_wt.iloc[:-1]))
    # break
#%%
...
g = sns.lmplot(x="stim", y="isChoiceLeft", hue="prevStim", data=mySessionLate.loc[mySessionLate.prevRwd, :],
               palette=sns.color_palette('YlOrRd', len(stim_set)), fit_reg=True, logistic=True,
               col='prev_long_wt',truncate=True,size=3.5,
               x_bins=stim_set,scatter=True, ci=None, scatter_kws={'alpha':.5})
# g.ax.set_title('After rewarded')
g.fig.show()
