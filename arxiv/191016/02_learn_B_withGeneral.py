...
from locallib import grad_beta, grad_rho, grad_l, grad_w, grad_phi

#%%
# def grad_lw(w,pRew,r,k,beta):
#     # delta_l = (1 - np.exp(-k / beta)) * expit(l) * (1 - expit(l)) * (2 * r - 1)
#     sig = pRew#expit(l)
#     exF = 1-np.exp(-k/beta)
#     delta_l = (1-sig) * (r - (1-r)*exF*sig/(1-exF*sig)) * w
#     return delta_l
...
Phi = np.eye(len(stim_set))
# Phi = np.random.randn(len(stim_set),int(2*len(stim_set)))
W = Phi.T.copy()
# L_hat = pd.Series(index=stim_set,dtype=float)

cols = ['isCorrect','isCatch','isRewarded','Reward','P(B)','est_P(B)','rho',
...
W[:] = np.random.randn(len(W.ravel())).reshape(W.shape)#np.eye(len(stim_set))
...
while (iTrial+1) < nTrials:
    if (iTrial + 1) % int(nTrials/100) == 0:
        print("iTrial:{:5.0f}, beta {:1.2f}, rho {:1.2f}, MSE est_P(B) {:1.2e}".format(iTrial + 1, betaHat, rhoHat,np.power((mySession.loc[:,'P(B)']-mySession.loc[:,'est_P(B)']).dropna(),2).mean()))
                                                                                   #mySession.loc[:,['P(B)','est_P(B)']].corr().iloc[0,1]))
    ...
    w = W[:,x]
    phi = Phi[x,:]
    pRew = expit(phi@w) # logistic transforms back to probability P_B.loc[x]#
    betaHat = mySession.loc[iTrial, 'beta']
    rhoHat = mySession.loc[iTrial, 'rho']
    f = mySession.loc[iTrial, 'feedbackTime']
    mySession.loc[iTrial, 'est_P(B)'] = pRew

    # A
    # temperature = 100 if iTrial < nTrials/2 else 0.1
    k = max(1e-6, betaHat * np.log(pRew / (betaHat * rhoHat)))
    ...
    # delta_w = grad_w(w, phi, r, k, betaHat)
    delta_phi = grad_phi(w, phi, r, k, betaHat)

    rhoHat += learnRateRho * delta_rho
    # w += learnRateB * delta_w
    phi += learnRateB * delta_phi
...
# hf_pp.fig.show()
#%%
hf_est, ha_est = plt.subplots(1,1,figsize=(3,3))
ha_est.plot(P_B.values,P_B.values,color='xkcd:gray',alpha=.5)
sns.regplot(x='P(B)', y='est_P(B)', ax=ha_est, truncate=True,
            data=mySessionLate, logistic=False,
            color='xkcd:dark blue', ci=None, x_bins=P_B.values, line_kws={'label': 'long WT'})

# g = sns.lmplot(x="P(B)", y="est_P(B)", hue="stim", data=mySessionLate,#.loc[mySessionLate.prevRwd, :],
#                palette=sns.color_palette('YlOrRd', len(stim_set)), fit_reg=True, logistic=False,
#                truncate=True,size=3.5,
#                x_bins=P_B,scatter=True, ci=None, scatter_kws={'alpha':.5})
hf_est.show()
...
#%
hf_wphi, ha_wphi = plt.subplots(3,1,figsize=(3,6))
ha_wphi[2].imshow(Phi@W)
ha_wphi[0].plot(Phi)
ha_wphi[1].scatter(P_B.values,expit(np.diag(Phi@W)))
hf_wphi.show()