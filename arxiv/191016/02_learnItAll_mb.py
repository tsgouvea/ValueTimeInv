...
sns.regplot(x='stim', y='isCorrect', data=mySessionLate, logistic=True, ci=None, ax=ha_learning[0,1],x_bins=stim_set,fit_reg=False)
xaxis = np.linspace(stim_set.min(),stim_set.max(),100)
mHat = mySessionLate.slope.mean()
bHat = mySessionLate.boundary.mean()
ha_learning[0,1].plot(xaxis,expit(mHat*abs(xaxis-bHat)),color='xkcd:silver',label='agents estimate')
