...
mySession.loc[:, 'percept'] = stim_set[np.argmin(abs(np.tile(stim_set,(nTrials,1))-np.tile((mySession.stim+np.random.randn(nTrials) * stim_noise).values.reshape(-1,1),(1,len(stim_set)))),axis=1)]
...
#%% Initialization
mySession.loc[0, 'rho'] = .1
mySession.loc[0, 'beta'] = .1
...
while (iTrial+1) < nTrials:
    ...
    c = mHat * (xHat - bHat) > 0
    k = np.random.choice(Phi.columns, 1,p=softmax(Phi.T @ theta / temperature)).item()
    optim_k = max(0, betaHat * np.log(eRew / (betaHat * rhoHat)))

    mySession.loc[iTrial, 'isChoiceLeft'] = c
    mySession.loc[iTrial, 'waitingTime'] = k
    mySession.loc[iTrial, 'optimWaitingTime'] = optim_k
