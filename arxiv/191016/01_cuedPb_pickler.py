...
from locallib import grad_beta, grad_psyc, grad_rho
#makephi, pnorm, truncExp
...
while (iTrial+1) < nTrials:
    if (iTrial + 1) % int(nTrials/100) == 0:
        print("iTrial:{:5.0f}, beta {:1.2f}, rho {:1.2f}".format(iTrial + 1, betaHat, rhoHat))
    ...
    # k = max(1e-6, betaHat * np.log(x / (1-x) * (1-(betaHat * rhoHat)) / (betaHat * rhoHat))) # LAK 14
